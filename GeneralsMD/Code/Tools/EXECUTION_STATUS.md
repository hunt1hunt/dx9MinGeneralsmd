...
| **2** | **重扫历史 `.spd`（共 729 份：712 污染 + 17 干净）** | 切分点 = 修复版 exe 部署时刻 **2026-09-17 10:42:14**（即 `E:\!!!!!!!QWCSB\RTSI.exe` 的 mtime），**早于此的 712 份首帧含累加污染**。先给 `spd_analyzer.py` / `plan_generator.py` 加"丢弃每份首行"兜底，再重算历史结论——**一切基于 max/峰值/卡顿聚类 的结论都要重过**（中位数结论不受影响）。 | 无需构建，可立即做 |
