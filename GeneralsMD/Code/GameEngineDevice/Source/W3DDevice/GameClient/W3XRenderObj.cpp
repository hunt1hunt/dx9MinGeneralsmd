...
{
	// 2026-09-11 PERF GATE (opt-1 redo): this UNBUFFERED log wrote WBONES
	// bursts every 3s per skinned object — sustained hot-path I/O. Off unless
	// the environment variable W3X_SHADOW_DIAG is set (debug sessions only).
	...
	IDirect3DBaseTexture9 *shadowTex = NULL)
{
	...
	// by zero in the shader when the map is unavailable.
	{
		...
		if (hTexel) {
			float smInvTexel = 1.0f / (float)((g_theW3DDeferredRenderer && g_theW3DDeferredRenderer->getShadowMapSize() >= 256) ? g_theW3DDeferredRenderer->getShadowMapSize() : 2048);
