	...
	// --- Base PBR shader: s0 + sun (GGX) ---
	{
		const char* src =
			...
			"    float m23 = step(22.5, shadowDbg.x) * (1.0 - step(23.5, shadowDbg.x)) * mViz;\n"
				"    if (m22 > 0.5) return tex2D(s4, suv).r;\n"
				"    if (m23 > 0.5) return saturate((sd - tex2D(s4, suv).r) * 8.0 + 0.5);\n"
			...
			// uncompensated tex0 the base uses - on cliffs that UV set is
			// stretched 3-6x, so each atlas texel row spans several screen
			// pixels (horizontal stripes) and the per-cell quadrant repeat
